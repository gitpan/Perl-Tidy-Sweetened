package Perl::Tidy::Sweetened::Keyword::ClassDefinition;

# ABSTRACT: Perl::Tidy::Sweetened filter plugin to handle p5-mop class definition
...
sub emit_sub {
    ...
    return sprintf 'package %s %s #__%s %s', $subname, $brace, $self->marker,
      $id;
}
...
sub prefilter {
    ...
    $code =~ s{
        ...
        $keyword             \s+  # the "class" keyword
        (?<subname> \w+)     \s*  # the class name
...
sub postfilter {
    ...
    $code =~ s{
        ...
        package           \s+  # keyword was convert to package
